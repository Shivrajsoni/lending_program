...
pub struct Liquidate<'info> {
    ...
    /// CHECK: This is a Pyth price update account
...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()> { 
    ...
    let sol_feed_id = get_feed_id_from_hex(SOL_USD_FEED_ID).map_err(|_| ErrorCode::InvalidPriceFeed)?; 
    let usdc_feed_id = get_feed_id_from_hex(USDC_USD_FEED_ID).map_err(|_| ErrorCode::InvalidPriceFeed)?;

    let clock = Clock::get()?;
    let solana_clock = SolanaClock::from(clock);
    
    let sol_price = price_update.get_price_no_older_than(&solana_clock, MAXIMUM_AGE, &sol_feed_id)
        .map_err(|_| ErrorCode::InvalidPriceFeed)?;
    let usdc_price = price_update.get_price_no_older_than(&solana_clock, MAXIMUM_AGE, &usdc_feed_id)
        .map_err(|_| ErrorCode::InvalidPriceFeed)?;
